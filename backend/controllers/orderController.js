...
// Recompute prices server-side from the DB to prevent tampering.
const buildOrderTotals = async (items) => {
  let itemsPrice = 0;
  const orderItems = [];
  ...
  return { orderItems, itemsPrice: +itemsPrice.toFixed(2), shippingPrice, taxPrice, totalPrice };
};
...
export const createOrder = asyncHandler(async (req, res) => {
  ...
  const totals = await buildOrderTotals(orderItems);
...
// @route PUT /api/orders/:id/pay  (mark paid after Stripe confirmation)
